...
// Some of these method actually using a lot of Memory. It is possible that you have to increase your java heap space.
public class Volume {
    ...
    public Volume(String path) {
        ...
        if (list == null){
        	System.out.println("The given Volume path seems to be not correct. Please check the path.");
        	System.exit(1);
        }
    ...
    public String getAttribute(String key) {
        ...
        // if str == "<<no attribute found>>" the attribute isnt set in the header
    ...
    public Image getSlice(int i) {
        ...
        if (i >= size) {
            System.out.println("Out of range (Index: " + i + ", Slices: "
                    + size + "). The Last Image is returned instead ("+(size-1)+").");
    ...
    public void extractHeader() {
        ...
        if (excounter!=0){
        	System.out.println("DicomHeaderExtractor failed "+excounter+"/"+slices.size()+" times, because some ***.header files already exist.");
        }
    }
