...
//		// creating the output area
	...
	public void search() {
		...
		for (String test : filterStr.split("&")){
			...
			if (parts.length < 2) {
				outputArea.setText("Invalid filter component. Please use: <Keyword> = <Value>");
			...
			if (val.equals("")) {
				outputArea.setText("No value is given behind '='");
				return;
			}
		}
			...
			outer: for (File potentialDicom : list) {
				...
				if (!fullsearch && foundAImage) {
					...
					try {
						...
						outputArea.setText(outputArea.getText()+"\n"+potentialDicom.getParentFile().getAbsolutePath());
						repaint();
